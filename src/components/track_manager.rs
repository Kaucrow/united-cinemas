...
impl TrackManager {
    ...
    pub fn setup_track_handlers(
        ...
        peer_connection.on_track(Box::new(move |track, _, _| {
            ...
            Self::spawn_pli_sender(peer_conn_weak.clone(), track.ssrc());

            // Spawn track relay
            Self::spawn_track_relay(track, track_sender);
    ...
    fn spawn_track_relay(
        ...
        tokio::spawn(async move {
            ...
            debug!("Track relay started, waiting for RTP packets...");

            let mut packet_count = 0;
            while let Ok((rtp, _)) = track.read_rtp().await {
                packet_count += 1;
                if packet_count % 100 == 0 {
                    debug!("Relayed {} RTP packets", packet_count);
                }

                if let Err(err) = local_track.write_rtp(&rtp).await {
                    if Error::ErrClosedPipe != err {
                        error!("Track relay error: {}, stopping", err);
                        break;
                    } else {
                        warn!("Track relay closed pipe: {}", err);
                    }
                }
            }
            error!("Track relay ended unexpectedly");
        });
    }
}